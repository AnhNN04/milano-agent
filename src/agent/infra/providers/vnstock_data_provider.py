...
class VnStockData(BaseStockData):
        ...
        try:
            for symbol in symbols:
                try:
                    ...
                    logger.error(f"Failed to fetch realtime data for {symbol}: {str(e)}")
                    raise
    ...
    async def get_historical_data(self, symbols: List[str], start_date: str, end_date: str) -> Dict[str, Any]:
        """
        ...
        try:
            for symbol in symbols:
                try:
                    ...
                    if data is not None and not data.empty:
                        formatted_data = [
                            {k: v.strftime('%Y-%m-%d') if k == 'time' else v for k, v in item.to_dict().items() if k != 'ticker'}
