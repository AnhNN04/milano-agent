...
try:
    ...
    logger.error("vnstock library is not installed. Please install it with: pip install vnstock")
...
class VnStockAnalysis(BaseStockAnalysis):
    ...
    async def get_fundamental_ratios(self, symbols: List[str]) -> Dict[str, Any]:
            ...
            for symbol in symbols:
                try:
                    ratios_data = await self._run_sync_function(
                        vns.financial_ratio, symbol=symbol, report_range='quarterly', is_all=False
                    )
                    if ratios_data is not None and not ratios_data.empty:
                        formatted_ratios = self._format_financial_ratios(ratios_data, symbol)
                        results[symbol] = formatted_ratios
                    else:
                        logger.warning(f"No fundamental data found for symbol: {symbol}")
                        results[symbol] = self._create_empty_fundamental_data(symbol)
                except Exception as e:
                    logger.error(f"Failed to fetch fundamental ratios for {symbol}: {str(e)}")
                    results[symbol] = self._create_error_fundamental_data(symbol, str(e))
            return results
        except Exception as e:
    ...
    async def get_peers_comparison(self, symbols: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if comparison_data is not None and not comparison_data.empty:
                # Chuẩn hóa data gốc
                formatted_data = self._format_stock_ls_analysis(comparison_data)
                ...
                for indicator in indicators:
                    try:
                        ...
                        logger.warning(f"Lỗi khi tạo insight cho chỉ số {indicator}: {str(e)}")
                ...
                logger.warning(f"No comparison data found for symbols: {symbols}")
                return {"data": {}, "insights": []}

        except Exception as e:
            logger.error(f"Failed to fetch peer comparison for {symbols}: {str(e)}")
    ...
    async def get_industry_analysis(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if industry_data is not None and not industry_data.empty:
                ...
                logger.warning(f"No industry analysis data found for symbol: {symbol}")
                raise

        except Exception as e:
            logger.error(f"Failed to fetch industry analysis for {symbol}: {str(e)}")
            raise

    
    def _format_financial_ratios(self, ratios_data: Any, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            quarter_columns = [col for col in ratios_data.columns if col not in ['ticker', 'quarter', 'year']]
            ...
            for column in quarter_columns:
                ...
                quarter_data["pe"] = self._safe_get_numeric(column_data, ["priceToEarning", "PE"])
                quarter_data["pb"] = self._safe_get_numeric(column_data, ["priceToBook", "PB"])
                quarter_data["roe"] = self._safe_get_numeric(column_data, ["roe", "returnOnEquity", "ROE"])
                quarter_data["eps"] = self._safe_get_numeric(column_data, ["earningPerShare", "EPS"])
                quarter_data["market_cap"] = None
                quarter_data["revenue"] = self._safe_get_numeric(column_data, ["totalRevenue"])
                quarter_data["profit"] = self._safe_get_numeric(column_data, ["netIncome"])
                quarter_data["debt_ratio"] = self._safe_get_numeric(column_data, ["debtOnEquity", "equityOnLiability"])
                quarter_data["current_ratio"] = self._safe_get_numeric(column_data, ["currentRatio"])
                quarter_data["quick_ratio"] = self._safe_get_numeric(column_data, ["quickRatio"])
                quarter_data["gross_margin"] = self._safe_get_numeric(column_data, ["grossMargin"])
                quarter_data["net_margin"] = self._safe_get_numeric(column_data, ["postTaxOnToi", "netMargin"])
                quarter_data["roa"] = self._safe_get_numeric(column_data, ["roa", "returnOnAssets", "ROA"])
            ...
            logger.error(f"Failed to format fundamental ratios for {symbol}: {str(e)}")
    ...
    def _format_stock_ls_analysis(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        indicators = [idx for idx in df.index] #if idx in column_mapping.values()]
            ...
            for indicator in indicators:
                ...
                if pd.notna(value):
                    if indicator == 'Vốn hóa (tỷ)':
                        ticker_data[indicator] = int((value / 1)) if value > 0 else 0
    ...
    def _safe_get_numeric(self, data: Dict[str, Any], keys: List[str]) -> Optional[float]:
