...
class QdrantVectorStoreDB(BaseVectorStore):
    ...
    def __init__(self, ):
        ...
        try:
            ...
            logger.info(f"Creating new collection {collection_name} with vector_size {settings.qdrant.vector_size}.")
        ...
        self.filter_conditions = getattr(settings.qdrant, 'filter_conditions', None)
    ...
    async def delete_documents(self, document_ids: List[str]) -> bool:
        ...
        try:
            ...
            await asyncio.to_thread(
                self.client.delete,
                filter={"must": [{"key": "source", "match": {"value": doc_id}} for doc_id in document_ids]}
            )
