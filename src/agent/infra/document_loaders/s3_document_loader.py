...
class S3DocumentLoader(BaseDocumentLoader):
    ...
    def _initialize_chunk_strategies(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("Semantic splitter initialized with Bedrock Cohere embeddings")
    ...
    async def load_and_chunk_document(self, source: str) -> List[DocumentChunk]:
        ...
        if not self._is_supported_document(source):
            raise DocumentProcessingError(f"Unsupported file type for {source}")
        ...
        try:
            ...
            if not documents:
                raise DocumentProcessingError(f"No content loaded from {source}")
                ...
                try:
                    ...
                    for i, chunk in enumerate(chunks):
                        ...
                        metadata = DocumentMetadata(
                            ...
                            document_type=self._extract_document_type_from_path(source),
                            tags=self._extract_tags_from_path(source) + [f"chunk_strategy:{strategy_name}"],
                            language="vi",
                    ...
                    logger.info(f"Applied {strategy_name} chunking strategy to {source}, produced {len(chunks)} chunks")
                except Exception as e:
                    logger.warning(f"Failed to apply {strategy_name} chunking strategy to {source}: {str(e)}")
                    continue
            
            if not result:
                raise DocumentProcessingError(f"No chunks produced for {source} with any strategy")
            
            return result
        
        except Exception as e:
            logger.error(f"Failed to load and chunk document {source}: {str(e)}")
            raise DocumentProcessingError(f"Failed to load and chunk document: {str(e)}")
    ...
    async def load_all_documents(self) -> List[DocumentChunk]:
        ...
        try:
            ...
            if not documents:
                logger.warning(f"No documents found in S3 bucket {self.bucket_name} with prefix {self.documents_prefix}")
                return []
            ...
            for strategy_name, strategy in self.chunk_strategies.items():
                try:
                    ...
                    logger.info(f"Applied {strategy_name} chunking strategy to all documents, produced {len(chunks)} chunks")
                except Exception as e:
                    logger.warning(f"Failed to apply {strategy_name} chunking strategy to documents: {str(e)}")
                    continue
            
            if not result:
                logger.warning(f"No chunks produced for any documents with any strategy")
                return []
            ...
            raise DocumentProcessingError(f"Failed to load all documents: {str(e)}")
    ...
    async def list_documents(self, prefix: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            effective_prefix = prefix if prefix is not None else self.documents_prefix
            ...
            while True:
                ...
                response = await asyncio.to_thread(self.s3_client.list_objects_v2, **params)
            ...
            if not documents:
                logger.info(f"No supported documents found in S3 bucket {self.bucket_name} with prefix {effective_prefix}")
                return []
            
            logger.info(f"Found {len(documents)} supported documents in S3 bucket {self.bucket_name} with prefix {effective_prefix}")
            return documents
        except Exception as e:
            logger.error(f"Failed to list documents with prefix {prefix}: {str(e)}")
