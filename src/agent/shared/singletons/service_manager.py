...
from ...infra.embeddings.cohere_multilingual_v3_embedding import CohereV3Embedding
...
class ServiceManager:
    ...
    async def get_instance(cls) -> 'ServiceManager':
        ...
        async with cls._lock:
            if cls._instance is None:
                ...
                logger.debug("Returning existing ServiceManager singleton instance")
    ...
    def __init__(self):
        ...
        if ServiceManager._instance is not None:
            logger.warning("Attempt to create another ServiceManager instance - using singleton")
            return
    ...
    async def _initialize_services(self):
        ...
        try:
            ...
            default_embedding_provider = getattr(settings.embeddings, 'default_provider', 'cohere')
            self._embeddings = {
            ...
            logger.info(f"Default embedding provider {default_embedding_provider} initialized")
            ...
            default_chat_provider = getattr(settings.llm, 'default_provider', 'gemini')
            ...
            logger.info(f"Default chat provider {default_chat_provider} initialized")
            ...
            logger.info("Stock providers (VnStockData, VnStockAnalysis) initialized")

            self._stats["initialization_time"] = time.time() - start_time
            logger.info(f"All services initialized in {self._stats['initialization_time']:.2f} seconds")

        except Exception as e:
            logger.error(f"Failed to initialize services: {str(e)}", exc_info=True)
            raise RuntimeError(f"Service initialization failed: {str(e)}")

    async def get_embeddings(self, provider_name: Optional[str] = None) -> Dict[str, BaseEmbeddings]:
    ...
    async def get_default_embedding(self) -> BaseEmbeddings:
        ...
        provider_name = getattr(settings.embeddings, 'default_provider', 'cohere')
        if provider_name not in self._embeddings:
            logger.error(f"Default embedding provider {provider_name} not initialized")
            raise RuntimeError(f"Default embedding provider {provider_name} not initialized")
    ...
    async def get_chat_providers(self, provider_name: Optional[str] = None) -> Dict[str, BaseChat]:
    ...
    async def get_default_chat_provider(self) -> BaseChat:
        ...
        provider_name = getattr(settings.llm, 'default_provider', 'gemini')
        if provider_name not in self._chat_providers:
            logger.error(f"Default chat provider {provider_name} not initialized")
            raise RuntimeError(f"Default chat provider {provider_name} not initialized")
    ...
    async def close(self):
        ...
        try:
            ...
            logger.error(f"Error during ServiceManager cleanup: {str(e)}", exc_info=True)