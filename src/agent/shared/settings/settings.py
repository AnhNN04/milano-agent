...
class TavilySettings(BaseSettings):
    ...
    include_domains: list[str] = ["cafef.vn", "vneconomy.vn", "vietstock.vn", "investing.com"]
    timeout_seconds: int = 5

    model_config = SettingsConfigDict(env_prefix="TAVILY_", case_sensitive=False)


class VnStock(BaseSettings):
    timeout_seconds: int = 5
    interval: str = "1D" # Khoảng thời gian lấy dữ liệu (vd: '1d', '1wk', '1mo')

    model_config = SettingsConfigDict(env_prefix="VNSTOCK_", case_sensitive=False)
...
class EmbeddingsSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="EMBEDDINGS_", case_sensitive=False)
...
class RedisSettings(BaseSettings):
    ...
    db: int = 0
    url: Optional[str] = None  # Redis URL có thể override các settings khác
    pool_size: int = 10
