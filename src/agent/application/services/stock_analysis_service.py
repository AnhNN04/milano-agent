...
class StockAnalysisService:
    ...
    def __init__(self, agent: StockReActAgent, tools: Dict[str, CustomBaseTool], session_manager: LangChainRedisSessionManager):
        self.agent = agent
        self.tools = tools
    ...
    async def analyze(self, query: str, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if cached_result:
            logger.info(f"Cache HIT for query: '{query[:50]}...', session: {effective_session_id}")
            cached_result["metadata"]["session_id"] = effective_session_id
            cached_result["metadata"]["cached"] = True
            return cached_result
        
        logger.info(f"Cache MISS - Starting fresh analysis for query: '{query[:50]}...', session: {effective_session_id}")
        ...
        try:
            ...
            logger.info(f"Starting workflow execution for session: {effective_session_id}")
            final_result = await self.workflow_manager.run(query=query, session_id=effective_session_id)
            ...
            result = {
                ...
                "metadata": {
                    ...
                    "intermediate_results": self._serialize_intermediate_results(final_result["metadata"]["intermediate_results"]),
                    "cached": False,
                    "query_hash": query_hash,
                    "execution_time": final_result["metadata"]["execution_time"],
                    "plan": final_result["metadata"]["plan"],
                    "sub_goals": final_result["metadata"]["sub_goals"],
                    "reflection_notes": final_result["metadata"]["reflection_notes"]
                }
            }
            ...
            if result["metadata"]["success"]:
                await self.session_manager.cache_analysis(effective_session_id, query_hash, result)
                logger.info(f"Analysis cached for session: {effective_session_id}")
            
            return result
            
        except Exception as e:
            logger.error(f"Analysis failed - Query: '{query[:50]}...', Session: {effective_session_id}, Error: {str(e)}", exc_info=True)
            ...
            await self.session_manager.cache_analysis(effective_session_id, query_hash, error_result)
    ...
    async def _ensure_session(self, session_id: Optional[str]) -> str:
        ...
        if session_id:
            ...
            if session:
                ...
                await self.session_manager.create_session_with_id(session_id=session_id)
                return session_id
        else:
    ...
    def _serialize_intermediate_results(self, intermediate_results: list) -> list:
