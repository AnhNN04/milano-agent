...
class DocumentProcessingService:
    ...
    async def process_documents(self, s3_keys: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not success:
                raise DocumentProcessingError("Failed to add documents to vector store")
            
            processed_docs = len(set(chunk.metadata.source for chunk in all_chunks))
            logger.info(f"Successfully processed {processed_docs} documents with {len(all_chunks)} chunks")
            
            return self._create_result(processed_docs, len(all_chunks), [], time.time() - start_time)
    ...
    async def _process_specific_documents(self, s3_keys: List[str]) -> List[DocumentChunk]:
        """Process specific documents by S3 keys."""
        all_chunks = []
        
        for s3_key in s3_keys:
            try:
                chunks = await self.document_loader.load_and_chunk_document(s3_key)
    ...
    def _create_result(self, processed_docs: int, total_chunks: int, failed_docs: List[str], processing_time: float) -> Dict[str, Any]:
    ...
    async def list_available_documents(self, prefix: Optional[str] = None) -> List[Dict[str, Any]]:
