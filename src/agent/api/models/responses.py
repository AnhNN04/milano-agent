...
class ToolResultResponse(BaseModel):
    ...
    data: Optional[Dict[str, Any]] = Field(None, description="Tool result data in JSON format per tool specification")
...
class AgentResponse(BaseModel):
    ...
    tools_used: List[str] = Field(..., description="List of tools used during processing")
    ...
    query_hash: Optional[str] = Field(None, description="Hash of the original query for caching")
...
class DocumentProcessResponse(BaseModel):
    ...
    success: bool = Field(..., description="Whether document processing was successful")
    ...
    failed_documents: List[str] = Field(..., description="List of documents that failed to process")
    processing_time: float = Field(..., description="Total processing time in seconds")
    total_documents: Optional[int] = Field(None, description="Total number of documents attempted")
    success_rate: Optional[float] = Field(None, description="Success rate as percentage")
    average_chunks_per_doc: Optional[float] = Field(None, description="Average chunks per document")
    
    def __init__(self, **data):
        super().__init__(**data)
        if self.total_documents is None and hasattr(self, 'processed_documents') and hasattr(self, 'failed_documents'):
            self.total_documents = self.processed_documents + len(self.failed_documents)
        if self.success_rate is None and self.total_documents and self.total_documents > 0:
            self.success_rate = (self.processed_documents / self.total_documents) * 100
        if (self.average_chunks_per_doc is None and 
            self.total_chunks and self.processed_documents and self.processed_documents > 0):
            self.average_chunks_per_doc = self.total_chunks / self.processed_documents
...
class SessionHistoryResponse(BaseModel):
    ...
    session_id: str = Field(..., description="Session identifier")
    history: List[Dict[str, Any]] = Field(..., description="Conversation history as BaseMessage-compatible JSON")
...
class DocumentListResponse(BaseModel):
    ...
    documents: List[str] = Field(..., description="List of document identifiers/paths")
    ...
    has_more: Optional[bool] = Field(None, description="Whether more documents are available")
    next_prefix: Optional[str] = Field(None, description="Next prefix for pagination")
...
class ErrorResponse(BaseModel):
    ...
    details: Optional[Dict[str, Any]] = Field(None, description="Additional error details, including JSON parsing errors")
...
class SystemStatsResponse(BaseModel):
    ...
    total_conversations: int = Field(..., description="Total conversation messages")
