...
from ...shared.session.redis_session_manager import LangChainRedisSessionManager
from ...shared.logging.logger import Logger
from ...shared.exceptions.domain_exceptions import DocumentProcessingError, VectorStoreError
import time
...
async def query_agent(
    ...
    try:
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.error(f"Query processing failed in {elapsed_ms:.2f} ms: {str(e)}", exc_info=True)
        raise HTTPException(
...
async def load_documents(
    ...
    try:
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.info(f"Loaded {result['processed_documents']} documents in {elapsed_ms:.2f} ms")
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.error(f"Document processing failed in {elapsed_ms:.2f} ms: {str(e)}")
        raise HTTPException(
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.error(f"Unexpected error in document processing after {elapsed_ms:.2f} ms: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def list_documents(
    prefix: Optional[str] = Query(None, description="Filter documents by prefix"),
    limit: Optional[int] = Query(100, description="Maximum number of documents to return", ge=1, le=1000),
    doc_service: DocumentProcessingService = Depends(get_document_processing_service)
):
    ...
    try:
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.info(f"Listed {len(documents)} documents in {elapsed_ms:.2f} ms")
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.error(f"Failed to list documents in {elapsed_ms:.2f} ms: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=str(e)
        )
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.error(f"Failed to list documents in {elapsed_ms:.2f} ms: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def create_session(
    ...
    session_manager: LangChainRedisSessionManager = Depends(get_session_manager)
):
    ...
    try:
        ...
        created_at_dt = datetime.fromtimestamp(session_data.created_at, tz=timezone.utc)
        expires_at_dt = datetime.fromtimestamp(session_data.created_at + session_manager.default_ttl, tz=timezone.utc)
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.error(f"Failed to create session in {elapsed_ms:.2f} ms: {str(ve)}")
        raise HTTPException(
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.error(f"Failed to create session in {elapsed_ms:.2f} ms: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_session_info(
    ...
    try:
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.info(f"Retrieved session info for {session_id} in {elapsed_ms:.2f} ms")

        return {
            "session_id": session.session_id,
            "created_at": datetime.fromtimestamp(session.created_at, tz=timezone.utc),
            "last_accessed": datetime.fromtimestamp(session.created_at + session_manager.default_ttl, tz=timezone.utc),
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.error(f"Failed to get session info in {elapsed_ms:.2f} ms: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_session_history(
    ...
    try:
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.info(f"Retrieved {len(paginated_history)} messages for session {session_id} in {elapsed_ms:.2f} ms")
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.error(f"Failed to get session history in {elapsed_ms:.2f} ms: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def delete_session(
    ...
    try:
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.error(f"Failed to delete session {session_id} in {elapsed_ms:.2f} ms: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def health_check(
    ...
    try:
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.info(f"Health check response in {elapsed_ms:.2f} ms: {health_status}")
        return health_status
    except Exception as e:
        elapsed_ms = (time.time() - start_time) * 1000
        logger.error(f"Health check failed in {elapsed_ms:.2f} ms: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
