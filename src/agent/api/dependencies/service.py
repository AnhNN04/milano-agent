...
async def initialize_services():
    ...
    try:
        ...
        logger.info(f"Global services initialized in {time.time() - start_time:.2f} seconds")
    except Exception as e:
        logger.error(f"Failed to initialize global services: {str(e)}", exc_info=True)
...
async def cleanup_services():
    ...
    try:
        ...
        logger.error(f"Error during global services cleanup: {str(e)}", exc_info=True)

async def get_session_manager() -> LangChainRedisSessionManager:
    """Get global session manager singleton"""
    if _session_manager is None:
        logger.error("Session manager not initialized. Ensure initialize_services was called during startup.")
        raise RuntimeError("Session manager not initialized. Please check Redis connection and startup logs.")
...
async def get_service_manager() -> ServiceManager:
    ...
    if _service_manager is None:
        logger.error("Service manager not initialized. Ensure initialize_services was called during startup.")
        raise RuntimeError("Service manager not initialized. Please check startup logs.")
...
async def get_tools(
    ...
    stock_analysis_provider: BaseStockAnalysis = Depends(get_stock_analysis_provider),
    ...
    return {
        ...
        "tavily_search": TavilySearchTool(web_search_retriever=web_search_retriever),
        "stock_price": StockPriceTool(stock_data_provider=stock_data_provider),
        "fundamental_analysis": FundamentalAnalysisTool(stock_analysis_provider=stock_analysis_provider),
        "industry_analysis": IndustryAnalysisTool(stock_analysis_provider=stock_analysis_provider),
        "peers_comparison": PeersComparisonTool(stock_analysis_provider=stock_analysis_provider),
