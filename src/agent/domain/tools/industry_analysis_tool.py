...
class IndustryAnalysisToolInput(BaseModel):
    ...
    symbol: str = Field(..., description="The stock symbol for analysis (e.g., 'VCB').")


class IndustryAnalysisToolOutput(BaseModel):
    """Output schema for IndustryAnalysisTool."""
    data: Dict[str, Any] = Field(description="Analysis table, rankings, and industry info")
...
class IndustryAnalysisTool(CustomBaseTool):
    ...
    async def _execute_impl(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            industry_data = await self._provider.get_industry_analysis(normalized_symbol)
                                                                              
            return IndustryAnalysisToolOutput(
                data=industry_data
            ).model_dump()
    ...
    def to_formatted_context(self, output: Dict[str, Any]) -> str:
        ...
        symbol = output.get("data", {}).get("analysis_table", {}).get("symbol", "N/A")
        
        formatted_data = []
        for symbol, data in analysis_table.items():
            quarter = data.get("quarter", "N/A")
            industry_name = data.get("industry_info", {}).get("name", "ngành")
            formatted_data.append(f"Stock: {symbol}, Quarter: {quarter}, Industry: {industry_name}")
            metrics = data.get("metrics", {})
            for metric, value in metrics.items():
                if value != "N/A" and value is not None:
                    formatted_data.append(f"  {metric.upper()}: {float(value):.2f}")
        
        formatted_data.extend(["Insights:"] + insights)
        return prompt_template.format(symbol=symbol, data="\n".join(formatted_data) if formatted_data else "No data available")