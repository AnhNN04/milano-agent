...
class PeersComparisonToolOutput(BaseModel):
    ...
    data: Dict[str, Any] = Field(description="Comparison table, rankings, and peers data")
...
class PeersComparisonTool(CustomBaseTool):
    ...
    description: str = "Compare stocks with their market peers using financial ratios"
    ...
    async def _execute_impl(
        ...
        try:
            ...
            peers_data = await self._provider.get_peers_comparison(normalized_symbols)

            return PeersComparisonToolOutput(
                data=peers_data
            ).model_dump()
    ...
    def _validate_parameters(self, symbols: Union[str, List[str]]) -> None:
        ...
        if not isinstance(symbols, list) or len(symbols) < 2:
            raise ValueError(f"peers_comparison requires at least two symbols: {symbols}")
