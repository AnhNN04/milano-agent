...
class ChatTool(CustomBaseTool):
    ...
    name: str = "chat_llm"
    description: str = "Interact with an LLM to answer queries about Vietnamese stocks, market news, and financial data"
    ...
    async def _execute_impl(self, messages: List[BaseMessage]) -> Dict[str, Any]:
        ...
        try:
            logger.info(f"Using chat provider with messages: {enhanced_messages}")
            chat_response = await self._chat_provider.chat(messages=enhanced_messages)
            sanitized_response = self._apply_content_filter(chat_response.get("response", ""))
            ...
            return ChatToolOutput(response="").model_dump()  # Consistent return type
    ...
    def to_formatted_context(self, output: Dict[str, Any]) -> str:
        ...
        return f"LLM Response: {output.get('response', 'No response available')}"