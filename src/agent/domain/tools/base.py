...
class ToolState(BaseModel):
    ...
    context: Dict[str, Any] = Field(default_factory=dict, description="Additional context for graph state")
...
class CustomBaseTool(StructuredTool, ABC):
    ...
    async def _arun(self, *args, **kwargs) -> ToolResult:
        ...
        try:
            result_data = await self._execute_impl(**kwargs)  # Expected to return dict
            end_time = time.time()
            ...
            self.state = ToolState(
                ...
                context={"market": "VN"}  # Example: Add Vietnam market context
            )
            ...
            return ToolResult(
                ...
                data=result_data,
                metadata={
                    "state": self.state.model_dump()  # Serialize state for LangGraph
                }
            )
    ...
    def to_langgraph_node(self) -> Any:
        ...
        return {
            "name": self.name,
            "tool": self,
            "state_update": lambda result: {"tool_state": self.state.model_dump() if self.state else {}},
        }