...
class StockPriceToolInput(BaseModel):
    ...
    symbols: Union[str, List[str]] = Field(..., description="The stock symbol(s) for price retrieval (e.g., 'FPT', ['HPG', 'VCB']).")
    data_type: str = Field("realtime", description="Type of data to retrieve: 'realtime' or 'historical'.")
    start_date: Optional[str] = Field(None, description="Start date for historical data in YYYY-MM-DD format.")
    end_date: Optional[str] = Field(None, description="End date for historical data in YYYY-MM-DD format.")
...
class StockPriceTool(CustomBaseTool):
    ...
    description :str ="Retrieve realtime or historical stock price data for Vietnamese stocks"
    ...
    async def _execute_impl(
        ...
        try:
            ...
            self._validate_parameters(data_type, normalized_symbols, start_date, end_date)

            # Route to appropriate data retrieval method
            if data_type == "realtime":
                raw_data = await self._stock_data_provider.get_realtime_data(normalized_symbols)
    ...
    def _validate_parameters(
        self, data_type: str, symbols: List[str], start_date: Optional[str], end_date: Optional[str]
    ) -> None:
        """Validate input parameters."""
