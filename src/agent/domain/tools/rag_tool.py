...
class RAGToolOutput(BaseModel):
    ...
    knowledge_context: str = Field(description="Formatted context from retrieved documents")
    sources: List[Dict[str, Any]] = Field(description="Metadata of retrieved documents")
    scores: List[float] = Field(description="Similarity scores of retrieved documents")
...
class RAGTool(CustomBaseTool):
    ...
    description: str ="Search internal knowledge base for Vietnamese stock market information from uploaded documents"
    ...
    def to_langchain_retriever(self) -> BaseRetriever:
        ...
        class RAGToolRetriever(BaseRetriever):
            ...
            async def _aget_relevant_documents(self, query: str) -> List[Document]:
                results = await self.tool._rag_retriever.retrieve(query=query, max_results=self.tool.args_schema.max_results.default)
                filtered_results = [r for r in results if r.get("score", 0) >= self.tool.args_schema.min_score.default]
                return [
                    Document(
